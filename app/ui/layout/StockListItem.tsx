	...
	return (
		<>
			{style == "large" && <tr className="hover:bg-slate-200">
				<td className="text-left whitespace-nowrap py-2 px-3 border-b text-blue-700 cursor-pointer font-semibold" onClick={() => showStockDetails(data)}>{data.symbol}</td>
				...
				{handleRemoveStock  && <td className="flex items-center ml-4 cursor-pointer" onClick={() => handleRemoveStock(data)} >
					<IoClose size={26} className="text-red" />
				</td>}
			</tr>}
			...
			{style == "small" && <section className="px-2 py-3 flex flex-col border-b odd:bg-gray-100 hover:bg-gray-200">
				<h2 className="text-lg font-semibold text-navy-blue mb-2 space-x-3 cursor-pointer flex flex-row">
					<span className="px-3 py-1 bg-black text-white rounded-md" onClick={() => showStockDetails(data)}>{data.symbol}</span> 
					<span onClick={() => showStockDetails(data)}>{data.longName}</span>
				...
				<div className="flex flex-col odd:bg-gray-100 hover:bg-gray-200">
					{/* <div className="whitespace-nowrap font-semibold text-blue-700 cursor-pointer" >{data.symbol} - {data.longName}</div> */}
					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 p-3">
						...
						<div>Market cap: <span className="font-semibold">{Utils.formatNumberToK(data.marketCap)}</span></div>
